...
/**
 * Object used to construct and parse DNS-SD format TXT records. For more info
 * see <a
 * href="http://files.dns-sd.org/draft-cheshire-dnsext-dns-sd.txt">DNS-Based
 * Service Discovery</a>, section 6.
 */

public class TXTRecord {
	/*
	 * DNS-SD specifies that a TXT record corresponding to an SRV record consist
	 * of a packed array of bytes, each preceded by a length byte. Each string
	 * is an attribute-value pair.
	 * 
	 * The TXTRecord object stores the entire TXT data as a single byte array,
	...
	/**
	 * Set a key/value pair in the TXT record. Setting an existing key will
	 * replace its value.
	 * <P>
	 * 
	 * @param key
	 ...
	 *            <P>
	 * @param value
	 *            Value to be encoded into bytes using the default platform
	 *            character set.
	 */
	...
	// Insert a key-value pair at index
	{
		...
		System.arraycopy(oldBytes, insertion, fBytes, newLen - secondHalfLen,
				secondHalfLen);
		...
		if (value != null) {
			...
			System.arraycopy(value, 0, fBytes, insertion + keyBytes.length + 2,
					valLen);
		}
	}

	/**
	 * Remove a key/value pair from the TXT record. Returns index it was at, or
	 * -1 if not found.
	 */
	public int remove(String key) {
		...
		for (int i = 0; avStart < fBytes.length; i++) {
			...
			if (key.length() <= avLen
					&& (key.length() == avLen || fBytes[avStart + key.length()
							+ 1] == kAttrSep)) {
				...
				if (0 == key.compareToIgnoreCase(s)) {
					...
					System.arraycopy(oldBytes, avStart + avLen + 1, fBytes,
							avStart, oldBytes.length - avStart - avLen - 1);
					return i;
				}
			}
	...
	/**
	 * Return a key in the TXT record by zero-based index. Returns null if index
	 * exceeds the total number of keys.
	 */
	 ...
	 * <P>
	 * Returns null if index exceeds the total number of keys. Returns null if
		...
		if (avStart < fBytes.length) {
			...
			for (aLen = 0; aLen < avLen; aLen++) {
				if (fBytes[avStart + aLen + 1] == kAttrSep) {
					...
					System.arraycopy(fBytes, avStart + aLen + 2, value, 0,
							avLen - aLen - 1);
					break;
				}
			}
	...
	/**
	 * Converts the result of getValue() to a string in the platform default
	 * character set.
	 */
	...
	/**
	 * Get the value associated with a key. Will be null if the key is not
	 * defined. Array will have length 0 if the key is defined with an = but no
	 * value.
	 * <P>
	 * 
	 * @param forKey
